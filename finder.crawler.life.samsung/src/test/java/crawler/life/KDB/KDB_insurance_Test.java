...
public class KDB_insurance_Test {
	...
	public void getKeys() throws Exception {
		List<String> keys = planService.getKeys("1506020001");
		...
		for (String key : keys) {
			...
			if (x <= 1) {
				...
				Plan plan = planService.getPlan("1506020001", key);
...
//				Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("(무) KDB다이렉트 암보험(갱신형)-나이")));
				...
				if (plan.getValue("(무) KDB다이렉트 암보험(비갱신형)-성별").equals("여자")) {
				...
				element = driver.findElement(By.id("pInsTerm"));	
				
				for (WebElement elementItem : element.findElements(By.tagName("option"))) {
					int elementSize = 1;
					tmp = plan.getValue("(무) KDB다이렉트 암보험(비갱신형)-납입기간").replace("년납", "");
				...
				String total = (String)driver.findElement(By.id("monthAmt1")).getText();
				...
				List<WebElement> rows = testTable.findElements(By.tagName("tr"));
				int t = 1;
...
*/
			...
			/*	String tmp = "";
				...
				WebElement  testEle= element.findElement(By.tagName("p"));
				testTotal = testEle.getText().replace("원", "");
				testTotal = testTotal.replace("납입시", "");
				testTotal = testTotal.replace("월", "");
				testTotal = testTotal.replace(",", "");
				testTotal = testTotal.trim();
...
/*				
			...
			/*	System.out.println("LongTest = " + testTotal);
				plan.setQuote(total);
					...
					String premium2 = "";
					String cancelTable = "";
					long tdLength =  (long) js.executeScript("return  $($('#mLayer1>div>div>div>table')[0]).find('tbody>tr>td').size();");
					int y = 1;
					for (long j = 0; j < tdLength; j++) {
						
						premium = (String) js.executeScript("return  $($('#mLayer1>div>div>div>table')[0]).find('tbody>tr>td')["+j+"].innerHTML;");
					...
					premium2  = (String) js.executeScript("return  $($('#mLayer1>div>div>div>table')[2]).find('tbody>tr>td')[1].innerHTML;");
					cancelTable = (String) js.executeScript("return  $($('#mLayer1>div>div>div>table')[3]).html();");
				...
				/*	element =	driver.findElement(By.id("firstInsu"));
					element = element.findElement(By.className("btn"));
					...
					element = element.findElement(By.className("layerPopBd"));
					...
					for (WebElement row : rows) {
						...
						for (WebElement col : cols) {
							...
							if (col.getText().indexOf("만원") != -1) {
							String colPay = col.getText().replace("만원", "");
								elementVal.put(t + "", colPay.replace(",", ""));
								System.out.println("***********"+elementVal.get(t+""));
								//System.out.println("***********"+elementVal.size());
								t++;
							}
					...
					*/
					...
					for(WebElement col : list){
						if(col.getText().indexOf("월") != -1){
							String tmp = col.getText().replace("원", "");
							 tmp = tmp.replace("납입시", "");
							 tmp = tmp.replace("월", "");
							 totalList.put(t+"",tmp.replace(",", ""));
							 t++;
						}
					}
					...
					String tmp = element.getText().replace("원", "");
					 tmp = tmp.replace("납입시", "");
					 tmp = tmp.replace("월", "");
...
//			Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("암진단보험금-나이")));
			...
			/*System.out.println("text = = = = = = "+driver.findElement(By.name("pbirthday")).getText());
			if(driver.findElement(By.name("pbirthday")).getText().equals("")){
				System.out.println("널이니라");
			}*/
			Birthday birth = InsuranceUtil.getBirthday(age);
			System.out.println("생년월일" + birth.getYear() + birth.getMonth() + birth.getDay());
			driver.findElement(By.id("pYmd")).sendKeys(	birth.getYear() + birth.getMonth() + birth.getDay());
			System.out.println("생년월일2" + birth.getYear() + birth.getMonth() + birth.getDay());
...
public void executeTest() throws Exception {
	...
	WebDriver driver = new FirefoxDriver();
	List<String> keys = planService.getKeys("1506100002");
	...
	for (String key : keys) {
		...
		if (x <= 800) {
			...
			Plan plan = planService.getPlan("1506100002", "0000002500");
			System.out.println("total = " + keys.size());
//			Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("(무) KDB다이렉트 암보험(갱신형)-나이")));
//			Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("(무) KDB다이렉트 연금저축보험-나이")));
			Birthday birth = InsuranceUtil.getBirthday(30);
			System.out.println("생년월일" + birth.getYear() + birth.getMonth() + birth.getDay());
			driver.findElement(By.id("pYmd")).sendKeys(	birth.getYear() + birth.getMonth() + birth.getDay());
			...
			if (plan.getValue("(무) KDB다이렉트 연금저축보험-성별").equals("여자")) {
				...
				driver.findElement(By.id("pBeginAge")).sendKeys("65");
			...
			System.out.println("********" + driver.findElement(By.id("L10")).getText());
			//driver.findElement(By.id("pop-saving-refund")).findElement(By.tagName("table")).getText();
			String cancelTable = "";
			cancelTable ="<table>" +  (String) js.executeScript("return  $($('#pop-saving-refund')).find('table').html();") + "</table>";
			//cancelTable = driver.findElement(By.id("cancelRefund")).getAttribute("innerHTML");
