...
public class Life_KDB_Inernet_Terninsurance_Type_Not_renewal {
	...
	public static void main(String[] args) throws Exception {
		...
		final Logger logger = Logger.getLogger(Life_KDB_Inernet_Terninsurance_Type_Not_renewal.class);
		...
		String premium2 = "";
		try {
			keys = planService.getKeys("1506020001");
		...
		for (String key : keys) {
			//if (Long.valueOf(key) > 10577) {
				javascript = new JavascriptLibrary();
				try {
					plan = planService.getPlan("1506020001", key);

					System.out.println("key result = " + key.toString());
					
					Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-나이")));
					...
					if (plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-성별").equals("여자")) {
						...
						element = driver.findElement(By.id("pInsType2"));
						if (!element.isSelected()) {
							element.click();
						}
						...
						for (WebElement elementItem : element.findElements(By.tagName("option"))) {
							...
							tmp = plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-보험기간").replace("세", "");
						...
						for (WebElement elementItem : element.findElements(By.tagName("option"))) {
							int elementSize = 1;
							if(plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-납입기간").equals("전기납")){
								tmp = plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-보험기간").replace("세", "");
							}else{
								tmp = plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-납입기간").replace("년납", "");
							}
					...
					if(plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-가입금액").equals("5000")){
					...
					if(plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-가입금액").equals("10000")){
					...
					if(plan.getValue("(무) KDB다이렉트 정기보험(비갱신형)-가입금액").equals("20000")){
					...
					plan.setValue("(무) KDB다이렉트 정기보험(비갱신형) 보장내역-지급금액1", premium1);
					plan.setValue("(무) KDB다이렉트 정기보험(비갱신형) 보장내역-지급금액2", premium2);
					System.out.println("total = " + total);
					 driver.findElement(By.id("pYmd")).clear();
					 planService.putPlan("1506020001", plan);
