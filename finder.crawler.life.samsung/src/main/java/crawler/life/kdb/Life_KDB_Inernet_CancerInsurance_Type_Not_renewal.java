...
public class Life_KDB_Inernet_CancerInsurance_Type_Not_renewal {
	...
	public static void main(String[] args) throws Exception {
		...
		final Logger logger = Logger.getLogger(Life_KDB_Inernet_CancerInsurance_Type_Not_renewal.class);
		...
		try {
			...
			keys = planService.getKeys("1505270002");
		...
		for (String key : keys) {
			//if (Long.valueOf(key) > 10577) {
				javascript = new JavascriptLibrary();
				try {
					plan = planService.getPlan("1505270002", key);
					...
					if (plan.getValue("(무) KDB다이렉트 암보험(갱신형)-성별").equals("여자")) {
						...
						for (WebElement elementItem : element.findElements(By.tagName("option"))) {
							...
							tmp = plan.getValue("(무) KDB다이렉트 암보험(비갱신형)-보험기간").replace("년", "");
							tmp.replace("세", "");
					...
					if(plan.getValue("(무) KDB다이렉트 암보험(비갱신형)-가입금액").equals("1250")){
					...
					if(plan.getValue("(무) KDB다이렉트 암보험(비갱신형)-가입금액").equals("2500")){
