...
public class Life_Shinhan_Inernet_Terminsurance_Type_Basic {
	...
	public static void main(String[] args) throws Exception {
		...
		final Logger logger = Logger.getLogger(Life_Shinhan_Inernet_Terminsurance_Type_Basic.class);
		...
		try {
			keys = planService.getKeys("1505080002");

		} catch (Exception e) {
			logger.error("통신상 에서 오류가 발생");
		...
		for (String key : keys) {
			if (Long.valueOf(key) > 11777) {

				javascript = new JavascriptLibrary();
				try {
					plan = planService.getPlan("1505080002", key);
					...
					Birthday birth = InsuranceUtil.getBirthday(Integer.valueOf(plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-나이")));

					for (WebElement elementItem : element.findElements(By.tagName("option"))) {
						if (elementItem.getAttribute("text").equals(plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-성별"))) {
							elementItem.click();
							break;
						}
					}
					...
					for (WebElement elementItem : element.findElements(By.tagName("option"))) {
						int elementSize = 1;
						tmp = plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-보험기간").replace("년", "");
					...
					for (WebElement elementItem : element.findElements(By.tagName("option"))) {
						int elementSize = 1;
						tmp = plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-납입기간").replace("년납", "");
					...
					for (WebElement elementItem : element.findElements(By.tagName("option"))) {
						if (plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-납입주기").equals("년납")) {
							tmp = "매년";
						} else {
							tmp = "매월";
						}
					...
					String total = "";
					if (plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액").equals("5000")) {
						System.out.println("가입금액 = = =" + plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액"));
						element = driver.findElement(By.id("it61"));
						element.click();
						total = driver.findElement(By.id("it61")).getText().replace(",", "");
						plan.setValue("보장내용-지급금액1", plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액"));

					} else if (plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액").equals("10000")) {
						System.out.println("가입금액 = = =" + plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액"));
					...
					} else if (plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액").equals("15000")) {
						System.out.println("가입금액 = = =" + plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액"));
					...
					} else if (plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액").equals("20000")) {
						System.out.println("가입금액 = = =" + plan.getValue("(무)신한Smart인터넷 정기보험(기본형)-가입금액"));
					...
					planService.putPlan("1505080002", plan);
